...
export const WinterOutfit = ({ SeasonDescription, smallBrandName, TitleShadow, MidBackgroundColors }: Props) => {
...
return (
  ...
  >
    <TitleWrapper>
      {SeasonDescription.split("").map((item, index) => makeText(item, index))}
    </TitleWrapper>
    <MidImageContainer>
      <MidImage
        season={season}
...
const Wrapper = styled(motion.div)<{ MidBackgroundColors: string[] }>`
  background: linear-gradient(
    250deg,
    ${props => props.MidBackgroundColors[2]},
    ${props => props.MidBackgroundColors[3]}
  );
  width: 100%;
  height: 100%;
  display: flex;
